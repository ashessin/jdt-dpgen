...
public class IteratorQ implements Callable<DpArrayList<DpSource>> {
	...
	private static final Logger log = (Logger) LoggerFactory.getLogger(IteratorQ.class);
	...
	static class InputGroup {
		...
		@CommandLine.Parameters(index = "1", paramLabel = "ConcreteAggregateName",
			description = "The ConcreteAgregate class implements the Iterator creation interface to " +
		...
		@CommandLine.Parameters(index = "3", paramLabel = "ConcreteIteratorName",
			description = "The ConcreteIterator implements the Iterator interface, " +
